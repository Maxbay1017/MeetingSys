...
<script setup lang="ts" name="Test">
    ...
    const blinkTimes=ref<number[]>([]); //眨眼时间数组
    ...
    const generateReport = async () => {
        try {
            const response = await axios.post('http://192.168.1.8:8000/generate-pdf', {
                ...
                blinkCount: blinkCount.value,
                mouthOpenCount: mouthOpenCount.value,
                blinkTimes: blinkTimes.value, // 新增字段：眨眼时间
            }, {
                responseType: 'blob',
            });
            ...
            ws.value.onmessage=(event)=>{
                ...
                if(data.event){
                    if(data.event==='blink'){
                        ...
                        //记录眨眼时间
                        const currentTimestamp=Date.now();
                        const blinkTime=Math.floor((currentTimestamp-meetingStartTime.value)/1000);
                        blinkTimes.value.push(blinkTime);
                    }
